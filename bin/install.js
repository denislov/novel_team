...
const ANS_CODEX_MARKER = '# ANS Agent Configuration \u2014 managed by ai-novel-studio installer';
const ANS_CODEX_HOOKS_OWNERSHIP_PREFIX = '# ANS codex_hooks ownership: ';

// Copilot instructions marker constants
const ANS_COPILOT_INSTRUCTIONS_MARKER = '<!-- ANS Configuration \u2014 managed by ai-novel-studio installer -->';
const ANS_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /ANS Configuration -->';
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all ans: references → ans-)
  c = c.replace(/ans:/g, 'ans-');
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all ans: references → ans-)
  c = c.replace(/ans:/g, 'ans-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize ans: -> ans-.
  // This preserves rendered "next step" commands like "/ans-execute-phase 17".
  return content.replace(/ans:/gi, 'ans-');
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{ANS_ARGS}}');
...
- Treat all user text after the skill mention as \`{{ANS_ARGS}}\`.
- If no arguments are present, treat \`{{ANS_ARGS}}\` as empty.
...
## C. Tool Usage
Use these Cursor tools when executing ANS workflows:
...
function convertClaudeCommandToCursorSkill(content, skillName) {
  ...
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run ANS workflow ${skillName}.`;
  if (frontmatter) {
...
## C. Tool Usage
Use these Windsurf tools when executing ANS workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
function convertSlashCommandsToCodexSkillMentions(content) {
  let converted = content.replace(/\/ans:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$ans-${String(commandName).toLowerCase()}`;
  });
  converted = converted.replace(/\/ans-help\b/g, '$ans-help');
  return converted;
}
...
- Treat all user text after \`${invocation}\` as \`{{ANS_ARGS}}\`.
- If no arguments are present, treat \`{{ANS_ARGS}}\` as empty.

## B. AskUserQuestion → request_user_input Mapping
ANS workflows use \`AskUserQuestion\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
ANS workflows use \`Task(...)\` (Claude Code syntax). Translate to Codex collaboration tools:
...
function generateCodexAgentToml(agentName, agentContent) {
  ...
  const resolvedDescription = toSingleLine(
    extractFrontmatterField(frontmatterText, 'description') || `ANS agent ${resolvedName}`
  );
...
/**
 * Generate the ANS config block for Codex config.toml.
...
function stripCodexAnsAgentSections(content) {
  return content.replace(/^\[agents\.ans-[^\]]+\]\n(?:(?!\[)[^\n]*\n?)*/gm, '');
}

/**
 * Strip ANS sections from Codex config.toml content.
 * Returns cleaned content, or null if file would be empty.
 */
function stripAnsFromCodexConfig(content) {
  const eol = detectLineEnding(content);
  const markerIndex = content.indexOf(ANS_CODEX_MARKER);
  const codexHooksOwnership = getManagedCodexHooksOwnership(content);

  if (markerIndex !== -1) {
    // Has ANS marker — remove everything from marker to EOF
    let before = content.substring(0, markerIndex);
    before = stripCodexHooksFeatureAssignments(before, codexHooksOwnership);
    // Also strip ANS-injected feature keys above the marker (Case 3 inject)
  ...
  // No marker but may have ANS-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.ans-*] sections (from header to next section or EOF)
  cleaned = stripCodexAnsAgentSections(cleaned);
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + ANS_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# ANS codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + ANS_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${ANS_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}

function isLegacyAnsAgentsSection(body) {
...
function stripLeakedAnsCodexSections(content) {
  const leakedSections = getTomlTableSections(content)
    .filter((section) =>
      section.path.startsWith('agents.ans-') ||
      (
        section.path === 'agents' &&
        isLegacyAnsAgentsSection(content.slice(section.headerEnd, section.end))
      )
    );
...
/**
 * Merge ANS config block into an existing or new config.toml.
 * Three cases: new file, existing with ANS marker, existing without marker.
 */
function mergeCodexConfig(configPath, ansBlock) {
  // Case 1: No config.toml — create fresh
  if (!fs.existsSync(configPath)) {
    fs.writeFileSync(configPath, ansBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(configPath, 'utf8');
  const eol = detectLineEnding(existing);
  const normalizedAnsBlock = ansBlock.replace(/\r?\n/g, eol);
  const markerIndex = existing.indexOf(ANS_CODEX_MARKER);

  // Case 2: Has ANS marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any ANS-managed sections that leaked above the marker from previous installs
      before = stripLeakedAnsCodexSections(before).trimEnd();

      fs.writeFileSync(configPath, before + eol + eol + normalizedAnsBlock + eol);
    } else {
      fs.writeFileSync(configPath, normalizedAnsBlock + eol);
    }
    return;
  }

  // Case 3: No marker — append ANS block
  let content = stripLeakedAnsCodexSections(existing).trimEnd();
  if (content) {
    content = content + eol + eol + normalizedAnsBlock + eol;
  } else {
    content = normalizedAnsBlock + eol;
  }
...
/**
 * Repair config.toml files corrupted by pre-#1346 ANS installs.
...
/**
 * Merge ANS instructions into copilot-instructions.md.
 ...
 * @param {string} ansContent - Template content (without markers)
 */
function mergeCopilotInstructions(filePath, ansContent) {
  const ansBlock = ANS_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    ansContent.trim() + '\n' +
    ANS_COPILOT_INSTRUCTIONS_CLOSE_MARKER;

  // Case 1: No file — create fresh
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, ansBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(ANS_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(ANS_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has ANS markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + ANS_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
...
/**
 * Strip ANS section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was ANS-only).
 * @param {string} content - File content
 * @returns {string|null} - Cleaned content or null if empty
 */
function stripAnsFromCopilotInstructions(content) {
  const openIndex = content.indexOf(ANS_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = content.indexOf(ANS_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + ANS_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('ans-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex ANS install path (absolute, so subagents with empty $HOME work — #820)
  const codexAnsPath = `${path.resolve(targetDir, 'ai-novel-studio').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/ai-novel-studio prefix so path resolves to codex ANS install
    content = content.replace(/~\/\.claude\/ai-novel-studio\//g, codexAnsPath);
    content = content.replace(/\$HOME\/\.claude\/ai-novel-studio\//g, codexAnsPath);
  ...
  const ansBlock = generateCodexConfigBlock(agents, targetDir);
  mergeCodexConfig(configPath, ansBlock);
...
function convertClaudeToGeminiAgent(content) {
  ...
  // when they can't be resolved. ANS agents use ${PHASE}, ${PLAN}, etc. as
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false } = {}) {
  ...
  // Replace /ans:command with /ans-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/ans:/g, '/ans-');
 ...
 * OpenCode expects: command/ans-help.md (invoked as /ans-help)
 * Source structure: commands/ans/help.md
 * 
...
function copyFlattenedCommands(srcDir, destDir, prefix, mentionPathPrefix, shellPathPrefix, runtime) {
  ...
  // Remove old ans-*.md files before copying new ones
...
function listCodexSkillNames(skillsDir, prefix = 'ans-') {
...
function copyCommandsAsCodexSkills(srcDir, skillsDir, prefix, mentionPathPrefix, shellPathPrefix, runtime) {
  ...
  // Remove previous ANS Codex skills to avoid stale command skills.
...
function copyCommandsAsCursorSkills(srcDir, skillsDir, prefix, mentionPathPrefix, shellPathPrefix, runtime) {
  ...
  // Remove previous ANS Cursor skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsWindsurfSkills(srcDir, skillsDir, prefix, mentionPathPrefix, shellPathPrefix, runtime) {
  ...
  // Remove previous ANS Windsurf skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsCopilotSkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous ANS Copilot skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
/**
 * Recursively install ANS commands as Antigravity skills.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'ans')
 * @param {boolean} isGlobal - Whether this is a global install
 */
function copyCommandsAsAntigravitySkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous ANS Antigravity skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyWithPathReplacement(srcDir, destDir, mentionPathPrefix, shellPathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      jsContent = jsContent.replace(/ans:/gi, 'ans-');
...
/**
 * Clean up orphaned files from previous ANS versions
 */
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/ans-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to ans-statusline.js in v1.9.0
  ];
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'ans-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to ans-statusline.js in v1.9.0
    'ans-intel-index.js',  // Removed in v1.9.2
    'ans-intel-session.js',  // Removed in v1.9.2
    'ans-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old ANS statusline.js path
  // Only match the specific old ANS path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1ans-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/ans-statusline.js)`);
  }
...
/**
 * Uninstall ANS from the specified directory for a specific runtime
 * Removes only ANS-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling ANS from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  // 1. Remove ANS commands/skills
  if (isOpencode) {
    // OpenCode: remove command/ans-*.md files
    const commandDir = path.join(targetDir, 'command');
    if (fs.existsSync(commandDir)) {
      const files = fs.readdirSync(commandDir);
      for (const file of files) {
        if (file.startsWith('ans-') && file.endsWith('.md')) {
          fs.unlinkSync(path.join(commandDir, file));
          removedCount++;
        }
      }
      console.log(`  ${green}✓${reset} Removed ANS commands from command/`);
    }
  } else if (isCodex || isCursor || isWindsurf) {
    // Codex/Cursor/Windsurf: remove skills/ans-*/SKILL.md skill directories
    ...
    // Codex-only: remove ANS agent .toml config files and config.toml sections
    if (isCodex) {
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('ans-') && file.endsWith('.toml')) {
    ...
    // Codex: clean ANS sections from config.toml
    const configPath = path.join(targetDir, 'config.toml');
    if (fs.existsSync(configPath)) {
      const content = fs.readFileSync(configPath, 'utf8');
      const cleaned = stripAnsFromCodexConfig(content);
      if (cleaned === null) {
        // File is empty after stripping — delete it
        fs.unlinkSync(configPath);
        removedCount++;
        console.log(`  ${green}✓${reset} Removed config.toml (was ANS-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(configPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned ANS sections from config.toml`);
      }
    }
    }
  } else if (isCopilot) {
    // Copilot: remove skills/ans-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Copilot: clean ANS section from copilot-instructions.md
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
    if (fs.existsSync(instructionsPath)) {
      const content = fs.readFileSync(instructionsPath, 'utf8');
      const cleaned = stripAnsFromCopilotInstructions(content);
      if (cleaned === null) {
        fs.unlinkSync(instructionsPath);
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was ANS-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned ANS section from copilot-instructions.md`);
      }
    }
  } else if (isAntigravity) {
    // Antigravity: remove skills/ans-*/ directories (same layout as Copilot skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    if (fs.existsSync(ansCommandsDir)) {
      ...
      removedCount++;
      console.log(`  ${green}✓${reset} Removed commands/ans/`);
    }
  }

  // 2. Remove ai-novel-studio directory
  const ansDir = path.join(targetDir, 'ai-novel-studio');
  if (fs.existsSync(ansDir)) {
    fs.rmSync(ansDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed ai-novel-studio/`);
  }

  // 3. Remove ANS agents (ans-*.md files only)
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} ANS agents`);
    }
  }

  // 5. Remove ANS package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed ANS package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  if (isOpencode) {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove ANS permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('ai-novel-studio')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed ANS permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No ANS files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} ANS has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading ANS reference docs
 * This prevents permission prompts when ANS accesses the ai-novel-studio directory
...
function configureOpencodePermissions(isGlobal = true) {
  ...
  // Build the ANS path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const ansPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/ai-novel-studio/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/ai-novel-studio/*`;
  ...
  if (config.permission.external_directory[ansPath] !== 'allow') {
    config.permission.external_directory[ansPath] = 'allow';
    modified = true;
  }
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for ANS docs`);
}
...
const PATCHES_DIR_NAME = 'ans-local-patches';
const MANIFEST_NAME = 'ans-file-manifest.json';
...
function writeManifest(configDir, runtime = 'claude') {
  ...
  const isCursor = runtime === 'cursor';
  const isWindsurf = runtime === 'windsurf';
  const ansDir = path.join(configDir, 'ai-novel-studio');
  const commandsDir = path.join(configDir, 'commands', 'ans');
  const opencodeCommandDir = path.join(configDir, 'command');
  const codexSkillsDir = path.join(configDir, 'skills');
  const agentsDir = path.join(configDir, 'agents');
  const manifest = { version: pkg.version, timestamp: new Date().toISOString(), files: {} };

  const ansHashes = generateManifest(ansDir);
  for (const [rel, hash] of Object.entries(ansHashes)) {
    manifest.files['ai-novel-studio/' + rel] = hash;
  }
  if (!isOpencode && !isCodex && !isCopilot && !isAntigravity && !isCursor && !isWindsurf && fs.existsSync(commandsDir)) {
    const cmdHashes = generateManifest(commandsDir);
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/ans/' + rel] = hash;
    }
  }
  ...
  if (!isCodex && !isCopilot) {
    ...
    if (fs.existsSync(hooksDir)) {
      for (const file of fs.readdirSync(hooksDir)) {
        if (file.startsWith('ans-') && file.endsWith('.js')) {
...
/**
 * Detect user-modified ANS files by comparing against install manifest.
 * Backs up modified files to ans-local-patches/ for reapply after update.
 */
function saveLocalPatches(configDir) {
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified ANS file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'copilot')
      ...
      : runtime === 'codex'
        ? '$ans-reapply-patches'
        : runtime === 'cursor'
          ? 'ans-reapply-patches (mention the skill name)'
          : '/ans:reapply-patches';
    console.log('');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Save any locally modified ANS files before they get wiped
  saveLocalPatches(targetDir);

  // Clean up orphaned files from previous versions
  cleanupOrphanedFiles(targetDir);

  // OpenCode uses command/ (flat), Codex uses skills/, Claude/Gemini use commands/ans/
  if (isOpencode) {
    ...
    if (installedSkillNames.length > 0) {
      ...
      failures.push('skills/ans-*');
    }
  } else if (isCopilot) {
    const skillsDir = path.join(targetDir, 'skills');
    const ansSrc = path.join(src, 'commands', 'ans');
    copyCommandsAsCopilotSkills(ansSrc, skillsDir, 'ans', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('ans-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const ansSrc = path.join(src, 'commands', 'ans');
    copyCommandsAsAntigravitySkills(ansSrc, skillsDir, 'ans', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('ans-')).length;
      if (count > 0) {
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    // Remove old ANS agents (ans-*.md) before copying new ones
  ...
  const changelogDest = path.join(targetDir, 'ai-novel-studio', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'ai-novel-studio', 'VERSION');
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf) {
    // Write package.json to force CommonJS mode for ANS scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // and stamp the current ANS version into the hook version header
  ...
  if (isCodex) {
    ...
    try {
      ...
      if (hasEnabledCodexHooksFeature(configContent) && !configContent.includes('ans-update-check')) {
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'ai-novel-studio', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
...
if (process.env.ANS_TEST_MODE) {
  module.exports = {
    ...
    ANS_COPILOT_INSTRUCTIONS_MARKER,
    ANS_COPILOT_INSTRUCTIONS_CLOSE_MARKER,
    mergeCopilotInstructions,
    stripAnsFromCopilotInstructions,
...
if (hasGlobal && hasLocal) {
  ...
  if (!hasGlobal && !hasLocal) {
    console.error(`  ${yellow}--validate requires --global or --local${reset}`);
    process.exit(1);
  }
  const runtimes = selectedRuntimes.length > 0 ? selectedRuntimes : ['claude'];
  for (const runtime of runtimes) {
  ...
  if (!hasGlobal && !hasLocal) {
    console.error(`  ${yellow}--repair requires --global or --local${reset}`);
    process.exit(1);
  }
  const runtimes = selectedRuntimes.length > 0 ? selectedRuntimes : ['claude'];
  for (const runtime of runtimes) {
    // Repair is simply reinstalling over the current directory
    console.log(`  Repairing ANS for ${cyan}${runtime}${reset}...`);
    install(hasGlobal, runtime);
  }
...
} // end of else block for ANS_TEST_MODE
